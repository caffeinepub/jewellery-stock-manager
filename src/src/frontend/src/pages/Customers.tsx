...
function CustomerView({
  ...
  const [deletingTxId, setDeletingTxId] = useState<bigint | null>(null);
  const [confirmDeleteTxId, setConfirmDeleteTxId] = useState<bigint | null>(
    null,
  );
  const deleteSaleTx = useDeleteSaleTransaction();

  const handleDeleteSale = async (txId: bigint) => {
    setDeletingTxId(txId);
    try {
      await deleteSaleTx.mutateAsync(txId);
    } finally {
                      ...
                      {confirmDeleteTxId === tx.id ? (
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            className="text-xs px-1.5 py-0.5 rounded bg-destructive text-white font-semibold disabled:opacity-50"
                          ...
                          <button
                            ...
                            className="text-xs px-1.5 py-0.5 rounded bg-muted text-muted-foreground font-semibold"
                          >
                            No
                          </button>
                        </div>
                        ...
                        <button
                          ...
                          title="Delete sale (returns item to stock)"
                        >
